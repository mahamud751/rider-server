import Ride from "../models/Ride.js";
import { notifyRidersAboutNewRide } from "./sockets.js";
import { BadRequestError, NotFoundError } from "../errors/index.js";
import { StatusCodes } from "http-status-codes";
import {
  calculateDistance,
  calculateFare,
  generateOTP,
} from "../utils/mapUtils.js";

export const createRide = async (req, res) => {
  console.log("=== CREATE RIDE REQUEST ===");
  console.log("User:", req.user);
  console.log("Body:", req.body);

  const { vehicle, pickup, drop } = req.body;

  if (!vehicle || !pickup || !drop) {
    console.log("Missing required fields:", { vehicle, pickup, drop });
    throw new BadRequestError("Vehicle, pickup, and drop details are required");
  }

  const {
    address: pickupAddress,
    latitude: pickupLat,
    longitude: pickupLon,
  } = pickup;

  const { address: dropAddress, latitude: dropLat, longitude: dropLon } = drop;

  if (
    !pickupAddress ||
    !pickupLat ||
    !pickupLon ||
    !dropAddress ||
    !dropLat ||
    !dropLon
  ) {
    console.log("Incomplete pickup/drop details");
    throw new BadRequestError("Complete pickup and drop details are required");
  }

  const customer = req.user;

  try {
    const distance = calculateDistance(pickupLat, pickupLon, dropLat, dropLon);
    const fare = calculateFare(distance, vehicle);

    console.log("Calculated distance:", distance, "km");
    console.log("Calculated fare:", fare);

    const ride = new Ride({
      vehicle,
      distance,
      fare: fare[vehicle],
      pickup: {
        address: pickupAddress,
        latitude: pickupLat,
        longitude: pickupLon,
      },
      drop: { address: dropAddress, latitude: dropLat, longitude: dropLon },
      customer: customer.id,
      otp: generateOTP(),
    });

    await ride.save();
    console.log("Ride created successfully:", ride._id);

    const io = req.app.get("io");
    const rideForSocket = await Ride.findById(ride._id).populate(
      "customer rider",
    );
    if (rideForSocket && io) {
      notifyRidersAboutNewRide(io, rideForSocket);
    }

    res.status(StatusCodes.CREATED).json({
      message: "Ride created successfully",
      ride,
    });
  } catch (error) {
    console.error("Error creating ride:", error);
    throw new BadRequestError("Failed to create ride");
  }
};

export const acceptRide = async (req, res) => {
  const riderId = req.user.id;
  const { rideId } = req.params;

  if (!rideId) {
    throw new BadRequestError("Ride ID is required");
  }

  try {
    let ride = await Ride.findById(rideId).populate("customer");

    if (!ride) {
      throw new NotFoundError("Ride not found");
    }

    if (ride.status !== "SEARCHING_FOR_RIDER") {
      throw new BadRequestError("Ride is no longer available for assignment");
    }

    ride.rider = riderId;
    ride.status = "START";
    await ride.save();

    ride = await ride.populate("rider");

    // Notify the customer that ride was accepted
    req.io.to(`ride_${rideId}`).emit("rideUpdate", ride);
    req.io.to(`ride_${rideId}`).emit("rideAccepted");

    // Broadcast to ALL riders to remove this ride from their list
    req.io.to("onDuty").emit("rideNoLongerAvailable", { rideId });

    console.log(
      `Rider ${riderId} accepted ride ${rideId}. Broadcasting removal to all riders.`
    );

    res.status(StatusCodes.OK).json({
      message: "Ride accepted successfully",
      ride,
    });
  } catch (error) {
    console.error("Error accepting ride:", error);
    throw new BadRequestError("Failed to accept ride");
  }
};

export const updateRideStatus = async (req, res) => {
  const { rideId } = req.params;
  const { status } = req.body;

  if (!rideId || !status) {
    throw new BadRequestError("Ride ID and status are required");
  }

  try {
    let ride = await Ride.findById(rideId).populate("customer rider");

    if (!ride) {
      throw new NotFoundError("Ride not found");
    }

    if (!["START", "ARRIVED", "COMPLETED"].includes(status)) {
      throw new BadRequestError("Invalid ride status");
    }

    // Handle payment when ride is completed
    if (status === "COMPLETED" && ride.rider) {
      const User = (await import("../models/User.js")).default;

      // Add fare to rider's balance
      await User.findByIdAndUpdate(
        ride.rider._id,
        { $inc: { balance: ride.fare } },
        { new: true }
      );

      console.log(
        `Added ৳${ride.fare} to rider ${ride.rider._id} balance. Ride ${rideId} completed.`
      );
    }

    ride.status = status;
    await ride.save();

    req.io.to(`ride_${rideId}`).emit("rideUpdate", ride);

    res.status(StatusCodes.OK).json({
      message: `Ride status updated to ${status}`,
      ride,
    });
  } catch (error) {
    console.error("Error updating ride status:", error);
    throw new BadRequestError("Failed to update ride status");
  }
};

export const getMyRides = async (req, res) => {
  const userId = req.user.id;
  const { status } = req.query;

  try {
    const query = {
      $or: [{ customer: userId }, { rider: userId }],
    };

    if (status) {
      query.status = status;
    }

    const rides = await Ride.find(query)
      .populate("customer", "name phone")
      .populate("rider", "name phone")
      .sort({ createdAt: -1 });

    res.status(StatusCodes.OK).json({
      message: "Rides retrieved successfully",
      count: rides.length,
      rides,
    });
  } catch (error) {
    console.error("Error retrieving rides:", error);
    throw new BadRequestError("Failed to retrieve rides");
  }
};

export const getRiderStatistics = async (req, res) => {
  const riderId = req.user.id;

  try {
    // Get all rides for this rider
    const totalRides = await Ride.countDocuments({ rider: riderId });
    const completedRides = await Ride.countDocuments({
      rider: riderId,
      status: "COMPLETED",
    });
    const cancelledRides = await Ride.countDocuments({
      rider: riderId,
      status: "CANCELLED",
    });

    // Calculate total earnings from completed rides
    const earnings = await Ride.aggregate([
      {
        $match: {
          rider: req.user._id,
          status: "COMPLETED",
        },
      },
      {
        $group: {
          _id: null,
          totalEarnings: { $sum: "$fare" },
        },
      },
    ]);

    const totalEarnings = earnings.length > 0 ? earnings[0].totalEarnings : 0;

    res.status(StatusCodes.OK).json({
      message: "Rider statistics retrieved successfully",
      statistics: {
        totalRides,
        completedRides,
        cancelledRides,
        totalEarnings,
      },
    });
  } catch (error) {
    console.error("Error retrieving rider statistics:", error);
    throw new BadRequestError("Failed to retrieve rider statistics");
  }
};

export const getCustomerStatistics = async (req, res) => {
  const customerId = req.user.id;

  try {
    // Get all rides for this customer
    const totalRides = await Ride.countDocuments({ customer: customerId });
    const completedRides = await Ride.countDocuments({
      customer: customerId,
      status: "COMPLETED",
    });
    const cancelledRides = await Ride.countDocuments({
      customer: customerId,
      status: "CANCELLED",
    });

    // Calculate total spent on completed rides
    const spending = await Ride.aggregate([
      {
        $match: {
          customer: req.user._id,
          status: "COMPLETED",
        },
      },
      {
        $group: {
          _id: null,
          totalSpent: { $sum: "$fare" },
        },
      },
    ]);

    const totalSpent = spending.length > 0 ? spending[0].totalSpent : 0;

    res.status(StatusCodes.OK).json({
      message: "Customer statistics retrieved successfully",
      statistics: {
        totalRides,
        completedRides,
        cancelledRides,
        totalSpent,
      },
    });
  } catch (error) {
    console.error("Error retrieving customer statistics:", error);
    throw new BadRequestError("Failed to retrieve customer statistics");
  }
};
